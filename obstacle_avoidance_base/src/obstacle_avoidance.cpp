...
class ObstacleAvoidance {
    ...
    protected: // ROS Callbacks
        ...
        void scan_callback(const sensor_msgs::LaserScanConstPtr msg) {
            ...
            cv::resize(d_alpha_,d_alpha_,cv::Size(200,200));
        ...
        geometry_msgs::Twist findClosestAcceptableVelocity(const geometry_msgs::Twist & desired) {
            ...
            double min_v = std::max(-max_linear_velocity_, current_velocity_.linear.x - time_horizon_*max_linear_accel_);
            double max_v = std::min(max_linear_velocity_, current_velocity_.linear.x + time_horizon_*max_linear_accel_);
            double min_w = std::max(-max_angular_velocity_, current_velocity_.angular.z - time_horizon_*max_angular_accel_);
            double max_w = std::min(max_angular_velocity_, current_velocity_.angular.z + time_horizon_*max_angular_accel_);
            ...
            // Now build Va (inside Vs inter Vd) by iterating over the local map,
            // and find the most appropriate speed (best_v, best_w).
            ...
            for (unsigned int j=0;j<n_v;j++) {
                ...
                for (unsigned int i=0;i<n_w;i++) {
                    ...
                    Va(j,i) = occupancy_dalpha(time_horizon_*v, atan2(v,w));
                    if(Va(j,i) == FREE) {						
						scores(j,i) = exp(-(0.5)*(k_v_*SQR(v-desired.linear.x)+k_w_*SQR(w-desired.angular.z))) * 255;
            ...
            cv::resize(Va,Va,cv::Size(200,200));
            cv::imshow("Va",Va);
