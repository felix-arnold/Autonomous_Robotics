    ...
    paused(false), manualControl(true), joystick_topic("/teleop/twistCommand"), auto_topic("/mux/autoCommand"), base_frame("/bubbleRob"), reference_frame("/world")
{
    ...
    faceDetectSub = nh.subscribe("/faceDetect",1,&SimTasksEnv::faceDetectCallback,this);
...
const face& SimTasksEnv::getFaceInfo() const {
...
void SimTasksEnv::faceDetectCallback(const face msg) {
    detectedFaces = msg;
}
